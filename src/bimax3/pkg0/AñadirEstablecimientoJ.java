...
public class AñadirEstablecimientoJ {
    ...
    try{
        Connection con =  ConexionMySQL.getConexion();
        PreparedStatement ps = con.prepareStatement("DELETE FROM detallesfactura WHERE numeroestablecimiento=?");
        ps.setString(1, numeroestablecimiento);
        ps.executeUpdate();
        JOptionPane.showMessageDialog(null,"Registro guardado");
        }catch(SQLException e){
    ...
    try{
        Connection con =  ConexionMySQL.getConexion();
        PreparedStatement ps = con.prepareStatement("UPDATE detallesfactura SET numerofactura=? WHERE numeroestablecimiento=?");
